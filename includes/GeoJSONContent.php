 ...
 */
class GeoJSONContent extends TextContent {

	const DEFAULT_RECURSION_LIMIT = 3;

	function __construct( $text ) {
		parent::__construct( $text, 'GeoJSON' );
	}
	 ...
	 */
	static function expand( $schema, $recursionLimit = GeoJSONContent::DEFAULT_RECURSION_LIMIT ) {
		return array_map( function ( $value ) {
			if ( is_array( $value ) && $recursionLimit > 0 ) {
				if ( isset( $value['$ref'] ) ) {
					$value = GeoJSONContent::resolve( $value['$ref'] );
				}
				return GeoJSONContent::expand( $value, $recursionLimit - 1 );
			} else {
				return $value;
			}
		}, $schema );
	...
	/**
	 * @throws GeoJSONException: If invalid.
	 * @return bool: True if valid.
	 */
	function validate() {
		$schema = $this->getJsonData();
		if ( !is_array( $schema ) ) {
			throw new GeoJSONException( wfMessage( 'eventlogging-invalid-json' )->parse() );
		}
		return $schema;
	}
	...
	function preSaveTransform( Title $title, User $user, ParserOptions $popts ) {
		return new GeoJSONContent( efMapBeautifyJson( $this->getNativeData() ) );
	}
